...
3. 一般问题出在新版burp上，由于jdk的原因导致代码运行错误，可以下载相应jdk版本的插件

<img width="1234" alt="image" src="https://user-images.githubusercontent.com/48286013/171561756-2a74a18b-8ad0-47f8-a17a-309b98784046.png">

4. 点击识别无响应，可以将服务识别端`codereg.py`放置在VPS上，使用公网地址访问即可
...
# 23-如何确认问题？
1. 不能获取验证码，说明插件配置有问题，也可能是插件不适配该类型的验证码
2. 能获取到验证码，说明插件本身没问题；识别端返回错误或者不返回，极有可能是python服务运行有问题，或者启动了代理，可以尝试关闭代理、更换jdk等方式尝试解决
...
# 25-intruder模块无法正常爆破
1. 检查线程是否为1
2. 检查延时是否超过1秒，如果1秒不行，则直接加大成5秒，站点的响应速度不同，延时也并非一成不变的
